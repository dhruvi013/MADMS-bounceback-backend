...
TABLE_NAME = "sponsored_research"  # Make sure this matches your table in Supabase

# Enable CORS in your main app.py
...
def add_sponsored_research():
    try:
        ...
        record = {
            "Proj_id": str(data["Proj_id"]).strip(),
            "year": int(data["year"]),
            "title": str(data["title"]).strip(),
            "mentor": str(data["mentor"]).strip(),
            "approval_date": str(data["approval_date"]).strip(),
            "grant_sanctioned": int(data["grant_sanctioned"]),
        }
